...
const EventList = () => {
  ...
  const getAverage = (array) => {
    return (
      array.reduce((sum, currentValue) => sum + currentValue.stars, 0) /
      array.length
    );
  };
  ...
  const handleDeleteEvent = async (e, eventId) => {
    ...
    try {
      ...
      setEvents((prevItems) => {
        return prevItems.filter((item) => item._id !== eventId);
      });
    } catch (err) {
                ...
                <>
                  {auth.isAuthenticated() &&
                    (auth.isAuthenticated().user.isAdmin ||
                      clubs
                        .find((item) => item._id === event.club._id)
                        .leadership.some(
                          (leader) =>
                            leader.leadershipId ===
              ...
              <ListItemText
                primary={
                  <Grid container>
                    ...
                    <Grid>
                      <Rating
                        value={getAverage(event.rating) ?? 0}
                        precision={0.1}
                        readOnly
                        size="small"
                        sx={{ mt: 1, ml: 1 }}
