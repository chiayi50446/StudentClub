...
export default function EditClub(props) {
  ...
  const SaveClub = () => {
    update({ clubId: props.club._id }, club, { t: jwt.token })
      .then((data) => {
        if (data) {
          if (data.error) {
  ...
  const AddLeaderShip = () => {
    ...
    setClub((prevClub) => ({
      ...prevClub,
      leadership: [...prevClub.leadership, { leadershipId: "" }],
    }));
  };
      ...
      <Dialog open={open} onClose={handleRequestClose}>
        ...
        <DialogContent>
          ...
          <Typography variant="h6" inline="true" sx={{ mt: 1 }}>
            Leadership Info
            <IconButton
              aria-label="addLeaderShip"
              color="primary"
              disabled={club.leadership && club.leadership.length === 3}
        ...
        <DialogActions>
          ...
          <Button onClick={SaveClub}>Update</Button>
        </DialogActions>
      </Dialog>
    </span>
  );
